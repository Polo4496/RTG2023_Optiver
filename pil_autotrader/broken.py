...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            mu = 2 * TICK_SIZE_IN_CENTS
            delta = TICK_SIZE_IN_CENTS + mu
            ...
            elif mid_price_future - mid_price_etf <= epsilon and self.position > 0:
                self.ask_id = next(self.order_ids)
                # self.ask_price = int(etf_bid) // TICK_SIZE_IN_CENTS * TICK_SIZE_IN_CENTS
                print("loss:", self.position * (etf_bid - future_ask))
                self.pnl = 0
                self.send_insert_order(self.ask_id, Side.SELL, MIN_BID_NEAREST_TICK, self.position,
                                       Lifespan.GOOD_FOR_DAY)
                self.asks.add(self.ask_id)
            elif mid_price_etf - mid_price_future <= epsilon and self.position < 0:
                self.bid_id = next(self.order_ids)
                # self.bid_price = int(etf_ask) // TICK_SIZE_IN_CENTS * TICK_SIZE_IN_CENTS
